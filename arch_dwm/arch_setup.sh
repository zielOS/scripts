...
pacman -S unzip zip unrar btrfs-progs dosfstools wget curl neofetch upower zathura zathura-pdf-poppler python-pynvim neovim ripgrep fd zsh zsh-completions zoxide fzf fuse flatpak lazygit xdg-user-dirs xdg-user-dirs-gtk xdg-utils thunar tumbler thunar-volman thunar-archive-plugin thunar-media-tags-plugin gvfs libx11 libxinerama libxft freetype2 feh rofi dunst flameshot xsel xclip xfce4-power-manager xorg-xsetroot xorg-server nvidia-dkms nvidia-utils nvidia-settings mesa rng-tools haveged lynis firejail audit sysstat apparmor snapper snap-pac grub mlocate sxiv fdupes acpi tmux keepassxc tldr linux linux-headers
...
GRUB_CMDLINE_LINUX="cryptdevice=UUID=0562681b-e32b-43f0-845e-10101245eab7:cryptroot root=/dev/mapper/cryptroot"
